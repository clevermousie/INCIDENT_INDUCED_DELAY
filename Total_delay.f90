...
module variables
    ...
    integer,pointer,dimension(:) ::crash_year,crash_month1,crash_month2,crash_day1,crash_day2,crash_hour1,crash_hour2,crash_minute1,crash_minute2,crash_dow1,crash_doy1,crash_dow2,crash_doy2,overlap
    integer dow
...
!**************************************************READ CRASH DATA*****************************    
    ...
    do i = 1, crash_num
     ...
     !do i =1, 10
        ...
        write(*,*) I,"INCIDENT FREE PERIOD SELECTED"
                ...
                WRITE(*,*) I,"UP & DOWN STATIONS FOUND: UPSTREAM IS ",UP_STATION,"DOWNSTREAM IS ",DOWN_STATION,"MP",crash_mp(i),"between",station_mp(up_station),station_mp(down_station)
                ...
                do k= 1, 365
                    if ((doy_avail(k)==1).AND.((k.ne.69).OR.(crash_hour2(i).lt.2).or.(crash_hour1(i).gt.3))) then
                            ...
                            !else
                             ...
                             !   flow_da=flow_ua
                            !endif
                            write(fileid_out_fea,"(7(I6),2(F10.4))",advance='no') i,crash_doy2(i),k,crash_hour2(i),crash_minute2(i),up_station, down_station, station_coef,(flow_ub*(1-time_coef)+time_coef*flow_ua)*(1-station_coef)+station_coef*(flow_db*(1-time_coef)+time_coef*flow_da)
    ...
    deallocate(crash_year,crash_month1,crash_month2,crash_day1,crash_day2,crash_hour1,crash_hour2,crash_minute1,crash_minute2,crash_dow1,crash_dow2,crash_doy1,crash_doy2)
